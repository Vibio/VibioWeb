...
if (isset($image)) {
	...
	if (!preg_match('/sites\/default\/files/', $image)) {
		$urlimage = "/sites/default/files/uploads/" . $image;
		// new: 20111121 (erase note)
	} else {
		//die($image);
		$urlimage = "/$image";
  ...
  $urlimage = str_replace('%2526', '&', $urlimage);
	...
	$imagecached = theme('imagecache', 'product_main_product_page', $image, $alt, $imgtitle, $attributes);
	$image = "
...
//, $variant); // For example, the "Have" button, now to be "I already have this item"
...
if (isset($node -> amazon_data)) {
	...
	$external_it_link = t("Find it on !external_link.", array("!external_link" => l(t("Amazon"), $node -> amazon_data['detailpageurl'], array("absolute" => true, "attributes" => array('target' => "_blank")))));
...
if ($page) {
	...
	foreach (_product_get_product_owners($node->nid, $user->uid) as $type => $data) {
		...
		$uid_highlight = $_GET['highlight'];
		// no security, passing to boolean ONLY
		...
		if (!empty($data['results'])) {
			foreach ($data['results'] as $i => $result) {
				...
				if ($result['user']['uid'] == $uid_highlight) {
					...
					array_unshift($data['results'], $hold);
					// work despite non-object?
